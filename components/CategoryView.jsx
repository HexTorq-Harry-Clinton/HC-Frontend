...
export default function CategoryView({ products, sizes = [], clothTypes = [], colors = [], showToolbar = true, showSort = true, emptyTitle = "No pieces yet" }) {
  ...
  const selectCls = "mt-2 w-full border border-white/25 bg-white/5 px-3 py-2 text-sm text-[#f7f4ec] focus:border-gold focus:outline-none";
    ...
    <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
      <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
        <div className="flex items-center justify-between">
          ...
          <button onClick={reset} className="text-sm text-neutral-400 underline hover:text-gold">
            Reset
          </button>
        </div>
        {sizes.length > 0 && (
        ...
        <div>
          ...
          <div className="mt-2 flex gap-2">
            <input value={minPrice} onChange={(e) => setMinPrice(e.target.value)} type="number" placeholder="Min" className="w-full border border-white/25 bg-white/5 px-3 py-2 text-sm text-[#f7f4ec] placeholder:text-neutral-500 focus:border-gold focus:outline-none" />
            <input value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} type="number" placeholder="Max" className="w-full border border-white/25 bg-white/5 px-3 py-2 text-sm text-[#f7f4ec] placeholder:text-neutral-500 focus:border-gold focus:outline-none" />
          </div>
        </div>
      ...
      <div>
        {showToolbar && (
          <div className="mb-5 flex items-center justify-between">
            ...
            {showSort && (
              <select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort" className="border border-white/25 bg-white/5 px-3 py-2 text-xs uppercase tracking-widest text-[#f7f4ec] focus:border-gold focus:outline-none">
