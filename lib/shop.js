...
// know (admin-renamed/added Menu-Category, Menu-Sub-Category, or
// Style-Collection slugs). Returns a renderable descriptor or null.
...
export async function resolveDbSlug(slugParts) {
  ...
  if (!key) return null;
  try {
    const [cats, subs, cols] = await Promise.all([
